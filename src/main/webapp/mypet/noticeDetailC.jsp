...
<table id="tableMenu">
		<tr>
			...
			<td> <button onclick="location.href='nnoteUpdatePage?mp_no=${nnotebook.mp_no}'"> 수정하기 </button> </td>
			<td> <button onclick="deletenn(${nnotebook.mp_no});">삭제하기</button> </td>
		</tr>
</table>

<hr>
<form action="commentC" name ="myForm" onsubmit="return call()"method="post">
	<table id="tableMenu" >
		<tr hidden="hiden">
...
<table id ="tableMenu" >
		...
		<tr>
			...
			<c:choose>
					<c:when test="${n.c_writer eq sessionScope.taccountInfo.t_name}">
						<td><button type="button" onclick="updatecm('${nnotebook.mp_no}','${n.c_no}','${n.c_txt}');">수정</button></td>
						<td><button type="button" onclick="deletecm(${nnotebook.mp_no},${n.c_no});">삭제</button></td>
					</c:when>
			</c:choose>	
		</tr>
		</c:forEach>
...
<td>
        ...
        <c:when test = "${pg > block}">
            [<a class="b" href="nnoteDetailC?pg=1">◀◀</a>]
            [<a class="b" href="nnoteDetailC?pg=${fromPage -1}">◀</a>] 
        </c:when>
        ...
        <c:forEach var="i" begin="${fromPage }" end="${toPage }">
        	...
        	<c:otherwise>
        		[<a class="b" href="nnoteDetailC?pg=${i }&mp_no=${nnotebook.mp_no}">${i }</a>]
        	</c:otherwise>
        	</c:choose>
        ...
        <c:choose>
        	<c:when test = "${toPage < allPage }">
        		[<a class="b" href="nnoteDetailC?pg=${toPage + 1}">▶</a>]
                [<a class="b" href="nnoteDetailC?pg=${allPage}">▶▶</a>]
        	</c:when>
        	<c:otherwise>
        		[<span style="color:gray">▶</span>]
                [<span style="color:gray">▶▶</span>]
